...
class FinalLayer(nn.Module):
    ...
    def __init__(self, hidden_size):
        ...
        self.linear = nn.Linear(hidden_size, hidden_size, bias=True)  #768 >> 2*2*4
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
...
class SiT(nn.Module):
    ...
    def initialize_weights(self):
        ...
        #     )
        #! change to 1d pos embedding
        positions = np.arange(self.num_patches, dtype=np.float32)
        pos_embed = get_1d_sincos_pos_embed_from_grid(
            self.pos_embed.shape[-1], positions
            )
    ...
    def forward(self, x, t, dir=None):
        
        x = x + self.pos_embed  # (N, P, D)
